...
def first_recommendation() -> Recommendation:
    ...
    return Recommendation(id=uuid4(), user_id=user_id, recommended_track_id=recommended_track_id)
...
def test_create_recommendation(recommendation_repo: RecommendationRepo,
    ...
    recommendation_repo.create_recommendation(first_recommendation.recommended_track_id, first_recommendation.user_id)
    recommendations = recommendation_repo.get_recommendations_for_user(first_recommendation.user_id)
    assert len(recommendations) == 1
    assert recommendations[0].recommended_track_id == first_recommendation.recommended_track_id
